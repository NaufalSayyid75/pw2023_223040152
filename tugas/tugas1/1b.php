<?php 
    $NRP = 52
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>tugas 1b</title>
</head>
<body>
    <p>
        Aku adalah angka <strong> <?php echo $NRP ?> </strong>
    </p>
    <p>
        Jika aku dikali 5, maka aku sekarang menjadi <strong> <?php echo $NRP = $NRP * 5 ?> </strong>
    </p>
    <p>
        Jika aku dibagi 2, maka aku sekarang menjadi <strong> <?php echo $NRP = $NRP / 2 ?> </strong>
    </p>
    <p>
        Jika aku ditambah 75, maka aku sekarang menjadi <strong> <?php echo $NRP = $NRP + 75 ?> </strong>
    </p>
    <p>
        Jika aku dikurangi 20, maka aku sekarang menjadi <strong> <?php echo $NRP = $NRP - 20 ?> </strong>
    </p>
</body>
</html>